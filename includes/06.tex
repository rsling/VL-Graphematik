...
%       \pause
%       \ex /kʁăçs/ $\Rightarrow$ [kʁaχs] (\textit{Krachs})
%       \pause
%       \ex /ăçt/ $\Rightarrow$ [ʔaχt] (\textit{Acht})
%     \end{xlist}
%     \pause
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item \ldots \rot{und im geschlossenen Einsilbler mit ungespannten Vokal}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \scalebox{0.7}{%
  \begin{tabular}{lllll}
    /k/  & k      & \rot{ck}        & \textit{Macke} & [maḳə] \\
    /t/  & t      & \alert{tt}      & \textit{Matte} & [maṭə] \\
    /p/  & p      & \alert{pp}      & \textit{Mappe} & [map̣ə] \\
      &&&\\
    /t͡ʃ/ & tsch ? & \orongsch{tsch} & \textit{Ratsche} & [ʁaṭ͡ʃə] \\
    ...
    /s/  & s      & \alert{ss}      & \textit{lassen} & [laṣən] \\
    /f/  & f      & \alert{ff}      & \textit{hoffen} & [hɔf̣ən] \\
      &&&\\
    /n/  & n      & \alert{nn}      & \textit{Wanne} & [vaṇə] \\
    /m/  & m      & \alert{mm}      & \textit{Kämme} & [kɛṃə] \\
    /l/  & l      & \alert{ll}      & \textit{knallen} & [knaḷən] \\
      &&&\\
    \grau{/g/} &&&& \\
    \grau{/d/} &&&& \\
    \grau{/b/} &&&& \\
