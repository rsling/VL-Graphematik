...
\begin{frame}
  ...
  \begin{exe}
    ...
    \ex \textit{untersetzt} /\tuerkis{ʊ}ntəʁz\alert{ɛ}t͡st/ $\Rightarrow$ [ʊn.tɐ.ˈz\orongsch{ɛ}t͡st]\\
    \ex \textit{motzte} /m\alert{ɔ}t͡stə/ $\Rightarrow$ [ˈm\orongsch{ɔ}t͡s.tə]\\
    \ex \textit{unglaublich} /\alert{ʊ}ngla͡ɔblɪç/ $\Rightarrow$ [\orongsch{ʊ}n.ˈgla͡ɔb.lɪç]\\
  \end{exe}
\end{frame}
...
\begin{frame}
  ...
  \blau{ungespannt + kurz (Akzent egal)} | \grau{Schwa, immer unbetont und immer kurz}
\end{frame}
...
\begin{frame}
  {Wozu dann Dehnungsschreibungen?}
  \onslide<+->
  \onslide<+->
  \begin{exe}
    \ex \textit{Beet}<++>
